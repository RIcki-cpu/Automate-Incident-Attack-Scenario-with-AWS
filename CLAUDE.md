...
- `manifest.yaml` — the scenario schema: `objectives`, `attack_path`, `detection` (source/query), `remediation`, `teardown`, and a top-level `status` field used as a lightweight state machine (`skeleton -> deployed -> attacked -> detected -> torn_down`). **Check `status` before assuming what's actually been done to a scenario** — it's the source of truth, more so than this file.
