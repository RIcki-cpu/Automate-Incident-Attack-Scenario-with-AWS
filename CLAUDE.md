...
There is no CI or lint step yet (`tflint` + GitHub Actions CI are a planned enhancement) — `terraform fmt`/`validate` are the only checks that currently exist.

**Terraform state:** scenarios use local state by default. An OPTIONAL S3 remote backend exists (`state-backend/` bootstrap + `scripts/remote-state.sh` + `docs/remote-state.md`): versioned/encrypted/private bucket named `tfstate-<account>`, S3-native locking (no DynamoDB). It's opt-in per scenario (writes a gitignored `backend.tf`), and needs the deploy policy's `tfstate-*` S3 permissions attached.

## Architecture: scenario packs

Each scenario lives under `scenarios/NN-name/` and is self-contained:
- `terraform/` — the infrastructure, flat (no shared modules yet — modules are deliberately deferred until real duplication genuinely exists across 2+ scenarios, not built speculatively)
...
## Planning documents (local-only, gitignored — not committed)

- `ROADMAP.md` and `aws-attack-defense-roadmap.md` — the working plan and original design notes. **Gitignored / local-only**, deliberately kept out of the public repo. Consult them for phase/status when they're present locally, but they are not part of the published project and their planning/timeline details should not be echoed into committed files.
...
Scenario 01 (S3 Data Exfiltration): **complete** — deployed, attacked (anonymous `GetObject`/`ListObjects`), detected (CloudTrail S3 data events, parsed via `scripts/detect.sh`), and torn down clean (zombie sweep verified). See `docs/technical-design.md` for the verified findings (a data-event activation-gap window right after `terraform apply`; anonymous `aws s3 ls` logs as `ListObjects`, not `ListBucket`). The instance has no SSH access or key pair by design (the attack is anonymous S3 access from the operator's own machine, and never touches the EC2 instance at all — the VPC/EC2 exist for narrative realism and the least-privilege contrast, not because the attack needs them); port 22 returns in Scenario 03, where Ansible needs SSH. Scenario 02 (IAM Privilege Escalation): **complete** — deployed, attacked (analyst user with blanket `sts:AssumeRole` + IAM recon escalates into `broad_read`, whose trust policy trusts account `:root`), detected (CloudTrail `AssumeRole` management event, no activation gap — contrast Scenario 01's data events), torn down clean. Deliberately no VPC/EC2 (identity-layer only). Two runtime gaps found and documented: `iam:ListGroupsForUser` is needed for `terraform destroy` to delete an IAM user (added to the policy; needs a Console re-attach before next deploy). See `docs/technical-design.md`. Scenario 03 (EC2 Lateral Movement): **complete** — deployed, attacked (foothold on public web host A → leaked pivot key → SSH pivot to private host B → read decoy), detected (VPC Flow Logs → CloudWatch Logs Insights, `scripts/detect.sh`, a deliberate contrast with scenarios 1/2's jq-over-S3 CloudTrail parsing), torn down clean. Vulnerability = one security-group gap (B's SG allows SSH from A's SG). Ansible (first use in the kit) configures the WEB host only; the internal host self-configures via Terraform `user_data` — deliberately, so nothing legitimately SSHes into B and the only A→B:22 flow is the attack (verified: web host was the sole source reaching B on 22). SSM Session Manager documented as the best-practice management path (remediation). Always confirm against each scenario's `manifest.yaml` `status` field rather than assuming.
