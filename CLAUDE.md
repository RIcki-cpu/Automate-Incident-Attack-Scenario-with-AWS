...
Scenario 01 (S3 Data Exfiltration): **complete** — deployed, attacked (anonymous `GetObject`/`ListObjects`), detected (CloudTrail S3 data events, parsed via `scripts/detect.sh`), and torn down clean (zombie sweep verified). See `docs/technical-design.md` for the verified findings (a data-event activation-gap window right after `terraform apply`; anonymous `aws s3 ls` logs as `ListObjects`, not `ListBucket`). The instance has no SSH access or key pair by design (the attack is anonymous S3 access from the operator's own machine, and never touches the EC2 instance at all — the VPC/EC2 exist for narrative realism and the least-privilege contrast, not because the attack needs them); port 22 returns in Week 2 when Ansible needs it. Scenario 02 (IAM Privilege Escalation): **complete** — deployed, attacked (analyst user with blanket `sts:AssumeRole` + IAM recon escalates into `broad_read`, whose trust policy trusts account `:root`), detected (CloudTrail `AssumeRole` management event, no activation gap — contrast Scenario 01's data events), torn down clean. Deliberately no VPC/EC2 (identity-layer only). Two runtime gaps found and documented: `iam:ListGroupsForUser` is needed for `terraform destroy` to delete an IAM user (added to the policy; needs a Console re-attach before next deploy). See `docs/technical-design.md`. Scenario 03 (EC2 Lateral Movement): Terraform written and `validate`-clean (`status: skeleton`) — two-tier VPC (public web host A + private internal host B), the vulnerability is one security-group gap (B's SG allows SSH from A's SG), **first scenario to use Ansible** (configures both hosts: leaked pivot key on A, decoy prize on B; reaches the private host B via ProxyJump through A). Detection is VPC Flow Logs → CloudWatch Logs Insights (`scripts/detect.sh`), a deliberate contrast with scenarios 1/2's jq-over-S3 CloudTrail parsing. Port 22 is back (Ansible needs SSH), as anticipated in Scenario 01's notes. Blocked on the bundled `iam/` policy re-attach (new `ScenarioFlowLogs` statement + key-pair/flow-log actions + broadened PassRole + pending `iam:ListGroupsForUser`) and a first real deploy to verify runtime specifics. Always confirm against each scenario's `manifest.yaml` `status` field rather than assuming.
